...
`publishedAt` 不可解析、**超出新鲜度窗口**、**与最近 7 天内某期重复（同一来源链接）**、
`total` 与实际条数不一致。
...
### 跨期查重（`dupLookbackDays`，默认 7 天）

分板块检索时最容易犯、又最难靠人工发现的错误，是同一件事被连着两天写进不同期，
或同一篇文章同时出现在两个板块。`validate` 会拿本期与最近 7 天的 `data/*.json` 逐条比对：

- **同一 `sourceUrl` → 错误**（就是同一件事又报了一遍）
- **标题相似度 ≥62% → 警告**（请人工确认是否同一事件）

发现重复时优先换掉较弱的那一条，而不是删掉了事。

> `check-links` 对每个链接先发 HEAD，失败再用 GET 复核。部分站点（如 `m.ithome.com`）
> 不支持 HEAD 会对 HEAD 直接返回 404 而 GET 正常，只发 HEAD 会造成误报。
